 ...
 * {@link UserDemoDAO} interface.
 *
 * @author luatnguyen
 */
public class UserDemoDAOJDBC implements UserDemoDAO {

	// Constants
	// ----------------------------------------------------------------------------------

	private static final String SQL_FIND_BY_ID = "SELECT id, email, firstname, lastname, birthdate FROM DemoUser WHERE id = ?";
	private static final String SQL_FIND_BY_EMAIL_AND_PASSWORD = "SELECT id, email, firstname, lastname, birthdate FROM DemoUser WHERE email = ? AND password = MD5(?)";
	private static final String SQL_LIST_ORDER_BY_ID = "SELECT id, email, firstname, lastname, birthdate FROM DemoUser ORDER BY id";
	private static final String SQL_INSERT = "INSERT INTO DemoUser (email, password, firstname, lastname, birthdate) VALUES (?, MD5(?), ?, ?, ?)";
	private static final String SQL_UPDATE = "UPDATE DemoUser SET email = ?, firstname = ?, lastname = ?, birthdate = ? WHERE id = ?";
	private static final String SQL_DELETE = "DELETE FROM DemoUser WHERE id = ?";
	private static final String SQL_EXIST_EMAIL = "SELECT id FROM DemoUser WHERE email = ?";
	private static final String SQL_CHANGE_PASSWORD = "UPDATE DemoUser SET password = MD5(?) WHERE id = ?";
	 ...
	 */
	UserDemoDAOJDBC(DAOFactory daoFactory) {
	 ...
	 */
	private UserDemo find(String sql, Object... values) throws DAOException {
		UserDemo user = null;
	...
	@Override
	public void update(UserDemo user) throws DAOException {
		if (user.getId() == null) {
	 ...
	 *             If something fails at database level.
	 */
	private static UserDemo map(ResultSet resultSet) throws SQLException {
